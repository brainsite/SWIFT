        ...
        for v in file_from_local:
            if (v[-3:]) == 'swl':
                ...
                try:
                    ...
                    logging.error(
                        u'Ошибка при перемещении ' + v + u' в ' + wk_out + u'. Причина:' + str(sys.exc_info()))
                try:
...
for x in listi_950:
    if x['listfile'] != []:
        for y in ['test_mail@in-bank.ru', 'test_mail2@in-bank.ru', 'test_mail3@in-bank.ru']:
            ...
            try:
                ...
                time.sleep(60)
                smtp.Send(subj='Сообщение не отправлено!!!' + x['title'], you='test_mail@in-bank.ru', Text=(x['text']))

for x in listi:
    if x['listfile'] != []:
        for y in ['test_mail@in-bank.ru', 'swift@in-bank.ru']:
            # for y in ['a.smirnov@in-bank.ru',]:
            # print(y)
            # smtp.Send(FileFolder=x['listfile'],subj=x['title']+' '+time.strftime('%Y.%m.%d'),you=y, Text=(x['text']))
            try:
